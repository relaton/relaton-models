...
One root `Gemfile` (lutaml-lml) and one root `Makefile`:
  ...
  make            # regenerate images/ from views/*.lutaml
...
* `models/`, `views/`, `images/`, `grammars/` — the shared RelBib models and
  base grammars (`biblio.rnc`, `biblio-standoc.rnc`, `biblio-compile.rnc`)
