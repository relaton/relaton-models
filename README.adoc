...
* https://github.com/metanorma/standoc-models[Metanorma document models]
...
models — never a substitute for them. A flavour directory containing only
an RNC overlay is incomplete: its model must exist as LML. This cannot be
violated.
====
...
https://github.com/metanorma/standoc-models/tree/main/grammars[Grammars]. The
base grammars for the BibliographicItem, Citation and Contributor models live
in `relaton/grammars/` here.

== Building

One root `Gemfile` (lutaml-lml) and one root `Rakefile`. Initialise the
`basicdoc` submodule first:

  git submodule update --init --recursive
  bundle install
  bundle exec rake render   # regenerate */images/ from */views/*.lml
  bundle exec rake verify   # assert PNG magic bytes on every committed diagram
  bundle exec rake parity   # assert LML models exist for every RNC overlay
  bundle exec rake check    # render + verify + parity
  bundle exec rake clean
  bundle exec rake site     # build the model catalog into _site/
  bundle exec rake lint     # semantic LML lint (names, types, view references)
  bundle exec rake fixtures # validate examples/ fixtures: XML vs RNC, YAML vs LML, YAML vs JSON Schema
  bundle exec rake profiles # validate profiles/*.yaml (narrowing-only)
  bundle exec rake schema   # regenerate the JSON Schema from the LML (relaton/schema/)

The workflow runs the same build on ubuntu.
A https://relaton.github.io/relaton-models/[model catalog site] is generated
from the views and deployed to GitHub Pages by the `pages` workflow.

== Architecture

This repository is the machine model for ISO 690, organized as:

* `relaton/` — shared RelBib base (BibliographicItem, Citation, Contributor)
* `citation/` — citation scheme/style/index/style models (as data, not code)
* `<flavour>/` — 29 SDO overlays (ISO, IEC, ITU, BSI, ...) with LML models + RNC grammars
* `profiles/` — declarative flavour profiles (narrowing-only)
* `examples/` — twin XML/YAML fixtures + real Relaton instances
* `mapping/` — CSL-to-Relaton subset mapping (CSL is a lower-level subset)
* `docs/VERSIONING.md` — model semver, deprecation cycle, ISO 690 snapshots

See https://relaton.github.io/relaton-models/[the model catalog] for browsable
diagrams, per-module detail pages, and a machine-readable inventory.

== Bibliography UML Models

=== Bibliographic Item

image::relaton/images/RelBib_BibliographicItem.png[]

=== Bibliographic Item Types

image::relaton/images/RelBib_BibItemType.png[]

=== Citation

image::relaton/images/RelBib_Citation.png[]

=== Contributor

image::relaton/images/RelBib_Contributor.png[]
image::relaton/images/RelBib_Contribution.png[]

=== Date

image::relaton/images/RelBib_Date.png[]

=== Document Relations

image::relaton/images/RelBib_DocumentRelation.png[]

=== Series

image::relaton/images/RelBib_Series.png[]

=== Keyword

image::relaton/images/RelBib_Keyword.png[]

=== Data Types

image::relaton/images/RelBib_DataTypes.png[]

=== Collection

image::relaton/images/RelBib_Collection.png[]
...
<bibdata type="standard">
  ...
  <uri type="xml">standards/csd-datetime-explicit/csd-datetime-explicit.xml</uri>
  <uri type="html">standards/csd-datetime-explicit/csd-datetime-explicit.html</uri>
  <uri type="relaton">standards/csd-datetime-explicit/cc-18001.xml</uri>
  ...
  <relation type="partOf">
    <bibitem type="standard">
      ...
      <status><stage>proposal</stage></status>
    </bibitem>
  </relation>
</bibdata>
----
...
== Repository layout — unified Relaton models

This repository consolidates the shared Relaton bibliographic models and every
former per-flavour `relaton-model-*` repository (relaton-model-ieee,
relaton-model-iso, ... 29 in total), each merged in under its own top-level
directory with its full git history preserved:

* `relaton/` — the shared RelBib base models and base grammars
* `basicdoc/` — git submodule of metanorma/basicdoc-models (Basicdoc types)
  (`biblio.rnc`, `biblio-standoc.rnc`, `biblio-compile.rnc`)
* `<flavour>/` — each former `relaton-model-<flavour>` repository: its models,
  views, images, and the flavour-specific grammar overlay
  `<flavour>/grammars/relaton-<flavour>.rnc`

Every module directory — the base (`relaton/`) and each flavour — uses the
same internal layout:

* `models/` — LutaML **definition modules** (`class` / `enum` / `data_type` only)
* `views/` — LutaML **diagram views** (one `diagram` each; `include`s + `association`s only)
* `images/` — rendered PNGs (committed; regenerated from views)
* `grammars/` — RNC implementation grammars accompanying the LML models

Views and models are a hard separation: never put class bodies in `views/`,
never put `diagram` / `association` in `models/`. Cross-module reuse is a
relative `include` of the real file — never a stub copy.


The Metanorma grammar hub
(https://github.com/metanorma/standoc-models[standoc-models]) consumes the
base grammars and flavour overlays from this repository.
