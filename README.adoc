...
One root `Gemfile` (lutaml-lml) and one root `Makefile`:

  make            # regenerate relaton/images/ from relaton/views/*.lutaml
  make verify     # assert PNG magic bytes on every committed diagram
  make clean
